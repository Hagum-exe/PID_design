...
############## throttle settings ###################
throttle_idle = 1040  #minimum throttle for four mototrs to spin up but not lift up
max_throttle = 2000  #maximum throttle limit
...
class PID_controller:
  ...
  def respond(self, sp, pv):    #pv as current position being passed in, sp is set point
    ...
    err = sp - pv
    P = self.kp * err
    I = self.ki*((self.I+err)/2) *cycle_time_seconds
...
def run():
  ...
  while rx_online==False:
    ...
    if pitch_input!=-225.0 and roll_input!=-225.0 and throttle_input!=0.0 and yaw_input!=-225.0 and arm_input!=-225.0:
      rx_online = True
